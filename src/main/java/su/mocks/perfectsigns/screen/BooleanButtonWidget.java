...
public class BooleanButtonWidget extends ButtonWidget implements TooltipUpdated {
  ...
  public void updateTooltip() {
    ...
    final MutableText content = value == null ? Text.empty().append(getSummaryMessage()) : ScreenTexts.composeToggleText(getSummaryMessage(), value);
    final MutableText narration = value == null ? Text.empty() : Text.translatable("narration.perfectsigns.button.current_value", value ? ScreenTexts.ON : ScreenTexts.OFF);
    if (tooltip != null) {
      content.append(ScreenTexts.LINE_BREAK).append(tooltip);
      narration.append(ScreenTexts.LINE_BREAK).append(tooltip);
    }
    if (keyboardShortcut != null) {
      MutableText composed = PerfectSignsUtils.describeShortcut(keyboardShortcut);
  ...
  public Text getMessage() {
    ...
    return value == null
        ...
        : Text.empty()
        .append(renderedName)
        .withColor(value ? 0xb2ff96 : 0xffac96);
  }
  ...
  protected void appendDefaultNarrations(NarrationMessageBuilder builder) {
    ...
    if (getValue() == null) {
      ...
      builder.put(NarrationPart.USAGE, Text.translatable("narration.perfectsigns.button.boolean_usage"));
    }
  }

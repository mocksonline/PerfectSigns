...
public class FloatButtonWidget extends ButtonWidget implements TooltipUpdated {
  ...
  public static final Float2ObjectFunction<MutableText> DEFAULT_VALUE_NARRATOR = value -> Text.literal(PerfectSignsUtils.numberToString(value));
  ...
  public void updateTooltip() {
    ...
    if (value != null) {
      ...
      setTooltip(Tooltip.of(ScreenTexts.composeGenericOptionText(getSummaryMessage(), valueText), Text.translatable("narration.perfectsigns.button.current_value", valueText)));
    } else {
      setTooltip(Tooltip.of(getSummaryMessage(), Text.empty()));
    }
  }
  ...
  public Text getMessage() {
    final Text summaryMessage = getSummaryMessage();
    final @Nullable Float value = getValue();
    final Text name = this.renderedNameSupplier == null ? summaryMessage
        : this.renderedNameSupplier.apply(value, valueToText.get(value == null ? 0 : value));
    return name == null ? summaryMessage : name;
  }
  ...
  protected void appendDefaultNarrations(NarrationMessageBuilder builder) {
    ...
    if (getValue() == null) {
      builder.put(NarrationPart.USAGE, Text.translatable("narration.perfectsigns.button.null"));
    } else {
      builder.put(NarrationPart.USAGE, Text.translatable("narration.perfectsigns.button.float_usage"));
    }
  }
